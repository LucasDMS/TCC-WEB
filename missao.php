...
<body>
	...
	<main>
		...
		<section class="section_missao">
			...
			<?php		
                $sql = "select * from tbl_texto_principal where  tipo_texto =  'Missao'";
				$stm = $con->prepare($sql);
				$success = $stm->execute();
			...
			<div class="missao_container">
                ...
                <?php		
                    $sql = "select * from tbl_missao_visao_valor where tipo_texto =  'Missao' and ativo = 1 and apagado = 0 limit 1";
				    $stm = $con->prepare($sql);
				    $success = $stm->execute();
				    foreach ($stm->fetchAll(PDO::FETCH_ASSOC) as $result){	
                ?>
				...
				<div>
                 ...
                 <?php		
                    $sql = "select * from tbl_missao_visao_valor where tipo_texto =  'Visao' and ativo = 1 and apagado = 0 limit 1";
				    $stm = $con->prepare($sql);
				    $success = $stm->execute();
				    foreach ($stm->fetchAll(PDO::FETCH_ASSOC) as $result){	
                ?>
